...
var commands = {
  ...
  'js ': {
    ...
    fn: (args) => {
      ...
      state.output = empty
      state.useMods = true
      return { text: state.subject + '/modson successfully.\n' }
    }
  }, 'modsoff': {
    ...
    fn: (args) => {
      ...
      state.output = empty
      state.useMods = false
      return { text: state.subject + '/modsoff successfully.\n' }
    }
  }, 'addmod ': {
...
function indexLast(str, substr, ...more) {
  let i = str.indexOf(substr, ...more)
  if (i < 0) {
    return -1
  }
